...
* `func`内で実行されたprint文は10を表示し、
* その後のグローバルスコープで実行されたprint文も10を表示

する。これは、関数`func`内で`print(a)`した時には`a`はグローバル変数として扱われているが、その後`a=20`が実行された時に、「新たなローカル変数`a`が定義され、それに20が代入された」と解釈されるためである。
