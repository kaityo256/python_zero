...
`continue`は、「以下の処理をスキップして、次のループに飛べ」という指示文である。やりたい処理を`if`文で囲むべきか、やりたくない処理を`continue`で飛ばすべきかは場合による。ループの中で`if`文の中身が大きくなったら、`continue`で書き換えることで`if`文が小さくすることを検討すると良い。
