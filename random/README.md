...
じゃんけんで「あいこ」になるのは、みんなのだした手がすべて同じ(1種類)か、全て異なる(3種類)かなので、出した手の種類の数を数えれば良い。

1. $N$人が「グー」「チョキ」「パー」のどれかを1/3の確率で選ぶ
2. 選んだ$N$個の「手」が、1種類か3種類の時には「あいこ」
3. 1と2を繰り返して確率を求める

というプログラムを組めば良い。ここで問題となるのは1.の「$N$人が「グー」「チョキ」「パー」のどれかを1/3の確率で選ぶ」というところだ。そのためには「グー」「チョキ」「パー」から「ランダム」にどれかを選ぶ、という処理が必要になる。このようなランダムな事象を扱うために、Pythonには`random`というライブラリが用意されている。今回のケースでは、リストの要素をランダムに選ぶ`random.choice`という関数を使えば簡単に実装できる。
...
とすると、実行するたびに`G`、`C`、`P`のどれかがランダムに選ばれる。これとリストの内包表記を使うと、例えば10人のじゃんけんの手のリストは、
...
なので、「合ってそうだな」ということがわかるだろう。

このように、プログラムで確率的な事象をシミュレーションして、何かの値を求める手法を **モンテカルロ法 (Monte Carlo Method)** と呼ぶ。
...
計算機において乱数が必要になることは多い。例えばゲームで低確率で出る「会心の一撃」や「痛恨の一撃」を表現するのに乱数が必要だ。レアなモンスターを出現させるのも乱数が必要である。しかし、現在の計算機は決定論的に動作するため「真の乱数列」を表現するのは難しい。「真の乱数列」とは、先程定義した通り「これまでの数列から、次の数字が予想できない」ような数列のことであるのに対し、計算機で実現される乱数は「疑似乱数」と呼ばれる。疑似乱数とは、一見乱数列のように見えるが実は規則性があり、これまでの乱数列から次の乱数が予想できてしまうものだ。

### Pythonにおける疑似乱数

#### `random.randint`

Pythonには擬似乱数用に`random`というライブラリが用意されており、様々な関数が用意されている。
...
これは`random.seed`により乱数の「種」を固定したためだ。計算機は、漸化式により乱数列を作ることが多い。漸化式は、生成した乱数を入力として次の乱数を作る方法だが、その一番最初に与える値を乱数の「種(seed)」と呼ぶ。同じ種からは同じ乱数列が生まれる。これでは乱数としては不都合であるので、「現在時刻」等を乱数の種とすることが多い。こうすると実行するたびに異なる乱数列が得られる。しかし、主にデバッグ目的などで、毎回同じ乱数の種を与えたいときもあり、`random.seed`は、そのような場合に用いる。乱数列を作る方法には、線形合同法やM系列など様々な方法があるが、現在広く使われているのはメルセンヌ・ツイスター法という手法である。多くのプログラム言語が乱数生成のデフォルトアルゴリズムとしてメルセンヌ・ツイスター法を採用している。本講義では擬似乱数生成アルゴリズムについては触れないが、興味がある人は調べてみると良い。また、原子核崩壊などの物理現象を用いることで「真の乱数」を作るデバイスも発売されている。
...
出現する乱数がすべて等しい確率である時、その乱数を **一様乱数** と呼ぶ。ある分布に従うような非一様な乱数が生成できると便利な場合もあるが、本講義では一様乱数のみを扱うことにする。

#### `random.choice`

リストが与えられた時、そのリストの要素をランダムに選びたいことがある。これはリストのサイズと`random.randint`を使って書くこともできるが、そのものずばりの`random.choice`という関数があるので、それを利用しよう。
...
簡単な「すごろく」を考えよう。例えば6マスのすごろくを考える。プレイヤーは6面サイコロを振り、出た目の数だけ進む。もしゴールに到着するか、行き過ぎてもゴールとしよう。例えば最初に5、次に2が出てもゴールである。プレイヤーがゴールするのに、平均で何回サイコロを振る必要があるだろうか？これくらいの問題なら理論値を求めるのは難しくないが、これがマスの数が多くなり、さらに「一回休み」や「3マス進む/戻る」といったマスがあると話がややこしくなる。「すごろく」がある程度以上複雑な場合はシミュレーションにより計算してしまった方が楽だ。

TODO: 説明

乱数を利用してシミュレーションをして、何か(特に確率)を計算する手法を **モンテカルロ法(Monte Carlo Method)** と呼ぶ。

いま、ある金融市場の予想をしたいとする。ある個別のプレイヤーについて株を「買う」か「売る」かは判断できない。しかし、これまでの履歴、つまり過去にどんな状況で株を「買った」か、もしくは「売った」かは知っているとする。すると、ある状況で株を「買う確率」と「売る確率」を推定することができる。同様に、別のプレイヤーについても「買う確率」と「売る確率」を推定し、その確率に従って仮想的に株を売り買いさせることで、その株が今後どのような値動きをするかシミュレーションすることができる。

モンテカルロ法は、数値積分にも用いられる。モンテカルロ法による数値積分で有名なのは、円周率の計算であろう。ダーツの要領でランダムに「矢」を投げ、当たった数で円周率を推定する方法である。以下は$n$回ダーツを投げて円周率を推定するプログラムである。
...
このアルゴリズムは簡単で、少ない試行回数でそこそこの精度が出るが、収束が悪いために円周率を高精度に求めるのには向かない。そもそも単純なモンテカルロ法はあまり使われないので、もし「モンテカルロ法はいい加減」とか「精度が悪い」と思っているのならモンテカルロ法による円周率の推定については忘れてほしい。
...
本講義ではモンテカルロ法を用いて、「モンティ・ホール問題」及び「パーコレーション」について調べてみる。
...
箱の中身のリストを渡されたときに、答えの箱と、プレイヤーの選ぶ箱をランダムに選び、一致するかどうかを返す関数`keep`を作ってみる。
