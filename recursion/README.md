...
単にゴールにたどり着くだけなら、「足跡」を残しながら進んでいけば良い。例えばこんなアルゴリズムになる。
...
行き止まりから戻る時に、まだ試してない道を見つけたら必ずそこを試すのがポイントである。これにより、「道の見落としが無い」ということが保証される。要するに「見落としが無いようにしらみつぶしに探しましょう」ということを言っているに過ぎない。

このように、とりあえず進めるだけ進んで、行き止まりに行き当たったら戻ってくる、という探索を **深さ優先探索(depth-first search)** と呼ぶ。逆に、自分がいまいる地点から徐々に探索範囲を広げながら探索する方法を **幅優先探索(breadth first search)** と呼ぶ。ここでは深入りしないが、「そういう用語がある」ということだけぼんやり覚えておけば良い。

さて、単にゴールにたどり着くだけなら上記の方法で良いが、ゴールまでの経路を求めるには一工夫が必要だ。
