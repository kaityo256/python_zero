...
条件分岐などで使われる。以下のうちどれか一つを選んでセルに入力し、実行せよ。なお、`#`以降はコメントであり、入力の必要はない。また、`#=>`とは、慣習として「その行を実行するとこの結果が出てくるよ」という意味である。例えば一行目の`True #=> True`は、`True`とだけ入力して実行すると、結果が`True`と帰ってくるという意味である。
