...
最初の`@jit`デコレータを忘れないこと。なお、関数の冒頭で`L, _ = u.shape`とあるのは、`u.shape`が2要素のタプルを返す際、そのうち最初の要素を`L`で受け取るが、2つ目は使わないことをアンダースコア「`_`」で表現している。宣言だけしてあとで使わない変数はバグの元なので、あとで使わないことがわかっている変数に対しては積極的に`_`を使って使わないことを明示するようにしたい。
