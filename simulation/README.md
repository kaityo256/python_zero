...
これが求めたい軌道であった。着弾するまでの時間は$r_y(t) = 0$となる$t$であるから、

$$
t = 0, \frac{2 v_0 \sin \theta}{g}
$$

である。$x$方向の速度はずっと$v_0 \cos \theta$であるから、着弾までに飛んだ距離$l$は

$$
l = \frac{2 v_0^2 \sin \theta \cos \theta}{g}
$$

これが最大になる角度は、$v_0$や$g$の値によらず$\theta = \pi/2$、すなわち45度であることがわかる。
...
まずは物を投げた場合の運動を、運動方程式を解くことで数値的に追ってみよう。新しいPython3ノートブックを開き、`ballistic.ipynb`という名前で保存せよ。
...
デフォルトサイズでは、ややグラフが小さいので、少しサイズを大きくする。
...
次に、台の上から投げてみよう。最初に高さ1の台に乗ってものを投げた場合にどうなるか調べてみる。
...
遠くに届く角度は45度から増えただろうか、減っただろうか。入力する角度を5度刻みで変化させ、最も遠くに届く角度を調べよ。
